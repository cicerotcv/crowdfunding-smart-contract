...
def expired_contract(CrowdFunding, accounts):
    ...
    contract.donate({"from": accounts[1], "value": 30})
...
class TestDonations:
    ...
    def test_donate_before_deadline(crowd_contract, accounts):
        ...
        crowd_contract.donate({'from': accounts[1], 'value': 10})
        # Directly access donations
        assert crowd_contract.donors(accounts[1].address)['donated_amount'] == 10

        # Donation Test
        crowd_contract.donate({'from': accounts[2], 'value': 20})
        # Directly access donations
        assert crowd_contract.donors(accounts[2].address)['donated_amount'] == 20
...
class TestClaim:
    ...
    def test_claim_donations(expired_contract, accounts):
        # goal unmet
        with brownie.reverts("Contract's goal must have been met for you to be able to claim donations"):
...
class TestRefund:
    ...
    def test_non_donor_refund_after_deadline(expired_contract, accounts):
        with brownie.reverts("Only donors are able to claim refund"):
